...
\date{Version 5.1\quad 2025/11/12}
\maketitle
...
\begin{figure}
  ...
  \framebox[\linewidth]{
    \begin{vertical}{17em}
      \linespread{1.5}\sffamily \verticalhangulfont
      \addhangulfontfeature{Renderer=Node, CharacterWidth=Full}
      \parindent0pt \everypar{\hangindent1em \hangafter1 }
  ...
  \caption{세로쓰기의 또 다른 예.  박스 높이 |17em|을 지시했다.
    ...
    반각 크기로 강제했다. 그런데 아스키 괄호를 전각으로 바꾸기 위해 지시한
    |CharacterWidth=Full| 옵션이 HarfBuzz 모드에서는 공백까지 전각으로 바꿔버리기 때문에
    |Renderer=Node| 옵션도 추가로 지시했다.
  }\label{fig:vertical2}
\end{figure}
