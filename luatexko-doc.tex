...
\date{Version 5.7\quad 2026/02/20}
\maketitle
...
영\hparen{零}으로 강제하기 위해서는 \pkgkwd{\inhibitglue} 명령을 사용한다.
...
\footnote{%
  ...
  하지만 |Renderer=Node|를 이용한 세로쓰기에 비해 아직은 실험 단계에 머물러 있다.
...
\kotex과 동일하게 항목 번호를 한국어 기호로 붙일 수 있다.
  ...
  이를테면 \cs{onum{page}}라고 하면 현재 쪽의 번호를 \onum{page}\로 찍는다.
