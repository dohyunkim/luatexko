...
\directlua{
  fonts.protrusions.setups.latin = {
    [0x28]   = { 0.5, 0   }, % (
    [0x29]   = { 0,   0.5 }, % )
    [0x2C]   = { 0,   1   }, % ,
    [0x2E]   = { 0,   1   }, % .
...
\setmainhangulfont{Noto Serif CJK KR}[
  ...
  UprightFont=* Light,
  BoldFont=* Bold,
  ItalicFont=Noto Sans CJK KR DemiLight,
  InterLatinCJK=.125em,
  CompressPunctuations,
  Expansion, Protrusion=latin,
]
\setsanshangulfont{Noto Sans CJK KR}[
  ...
  BoldFont=* Bold,
%  ItalicFont=Noto Serif CJK KR Light,
  InterLatinCJK=.125em,
  CompressPunctuations,
  Expansion, Protrusion=latin,
]
...
\protected\def\cs#1{\texttt{\textbackslash\detokenize{#1}}}
\protected\def\pkgkwd#1#{\leavevmode
  \ifx\relax#1\relax \expandafter\luatexkopkgkwd
  \else              \expandafter\luatexkopkgkwdX \fi}
\def\luatexkopkgkwd#1{\texttt{\string#1}\luatexkopkgkwdX{#1}}
\def\luatexkopkgkwdX#1{\marginpar{%
  \hfuzz\maxdimen \texttt{\footnotesize \string#1}}}
%\def\logoko{\textsf{k}\kern-.0625em\textit{o}} % too slow
\ExplSyntaxOn
\cs_new_nopar:Npn \actualtag #1 #2
{
  \leavevmode
  \tag_if_active:TF
  {
    \tag_mc_end_push:
    \tag_struct_begin:n{tag=Span, actualtext=#1}
    \tag_mc_begin:n{}
    \SuspendTagging{actualtag}
    #2
    \ResumeTagging{actualtag}
    \tag_mc_end:
    \tag_struct_end:
    \tag_mc_begin_pop:n{}
  }
...
\cs_new_nopar:Npn \artifacttag #1
{
  \leavevmode
  \tag_if_active:TF
  {
    ...
    \SuspendTagging{artifacttag}
    #1
    \ResumeTagging{artifacttag}
    \tag_mc_end:
    \tag_mc_begin_pop:n{}
  }
  {
...
\def\luatex{\hologo{LuaTeX}\unskip}
\def\XeTeX{\hologo{XeTeX}\unskip}
\def\luatexko{\luatex\artifacttag{-}\logoko}
\def\texlive{\hologo{TeXLive}\unskip}
\def\kotex{\actualtag{koTeX, phoneme=[ˌkeɪˈəʊtɛx]}{\logoko\kern-.1ex .\kern-.1667em \TeX}}
\def\hemph#1{\textsf{\bfseries #1}}
\def\meta#1{\actualtag{<#1>}{$\langle$\textit{#1}$\rangle$\nobreak\inhibitglue}}
\makeatletter
\def\fakespaceoff#1{\csname tag_spacechar_off:\endcsname#1\csname tag_spacechar_on:\endcsname}
\def\hparen#1{\begingroup\fontsize{.9\dimexpr\f@size pt\relax}\z@\selectfont
  \ifdim\lastskip>\z@\else \ifdim\lastkern>\z@\else
  \fakespaceoff{\hskip.1em plus.01em\relax}\fi\fi (#1)\futurelet\@let@token\hparenstop}
\def\hparenstop{\ifx\@let@token\@sptoken\else
  \ifx\@let@token\footnote\else\ifx\@let@token ,\else\ifx\@let@token '\else
  \fakespaceoff{\hskip.1em plus.01em\relax}\fi\fi\fi\fi\endgroup}
\newenvironment{plaintex}{%
  \par\smallskip
  \linespread{1.1}\small \narrower \parindent=0pt \parskip=2pt plus 1pt
  \topsep=\smallskipamount \@totalleftmargin=\leftskip
  \leavevmode\llap{\color{gray}\normalfont\hologo{plainTeX}\unskip\hskip1em}\ignorespaces
}{%
  \par
}
\newenvironment{sourceandresult}{%
  \par
  \if@nobreak \else \ifdim\lastskip>\z@ \else \medskip \fi \fi
  \narrower \parindent0pt
}{%
  \par\medskip
  \AddToHookNext{para/begin}{\OmitIndent}%
}
\def\srcres#1{\leavevmode \texttt{\srcres@#1\stop}\quad ⇒\quad #1\par }
\def\srcres@#1#2#3\stop{%
  \if\noexpand#1\relax \string#1\{#2\}\ifx\empty#3\empty\else\{#3\}\fi
  \else \detokenize{#1#2#3}\fi
}
\makeatother
\def\ldots{|...|}
...
\author{\normalsize Dohyun Kim \small |<nomosnomos at gmail com>| \and
        \normalsize Soojin Nam \small |<jsunam at gmail com>| \and
        \normalsize <\url{http://github.com/dohyunkim/luatexko}>}
\date{Version 5.5\quad 2026/01/26}
\maketitle
...
BOM \hparen{Byte Order Mark}은 있어도 좋고 없어도 좋다.
...
장\hparen{chapter} 제목과 편\hparen{part} 제목에만 ``제\hparen{第}''가 붙어 ``제~1 편''
...
\begin{plaintex}
  ...
  명시적 설정이 없으면 \hemph{은바탕}을 기본으로 로드한다.
\begin{verbatim}
...
폰트에서 글자를 찾아 찍는다.%
\footnote{%
  이 기능이 작동하지 않아 한글이 식자되지 않는다면 최후수단으로
  \cs{luatexkohangulfont}를 선언할 수 있다.
  당해 지점의 한글폰트를 메인 폰트로 잠시 지정하는 명령이다.
  \cs{luatexkohanjafont}와 \cs{luatexkofallbackfont}도 각각 상응하는 기능을 한다.
}
...
\footnote{%
  정확히 말하자면 |\\language=\\l@nohyphenation| \hparen{플레인텍에서는 \cs{language=-1}인 경우},
...
\footnote{%
  ...
  \hparen{플레인텍이라면 |script=hang|}을 주어야 한다.
  옛한글이 아니더라도 한글폰트에는 이 옵션과 |Language=Korean|
  \hparen{플레인텍은 |language=KOR|}을 주는 것이 좋다. }
...
\begin{figure}
  ...
  \caption{옛한글 조판 보기. Noto Sans CJK KR 폰트에
    |Script=Hangul|과 |Language=Korean| 옵션을 주었다.
    비록 이름에 KR이 들어있지만, |Script|와 |Language| 옵션만 적절히 지시한다면
    이 폰트는 모든 CJK 언어를 잘 지원한다.}\label{fig:yethangul}
\end{figure}

\begin{description}
    \topsep=6pt plus 1pt minus 1pt
    \parskip=1pt plus 1pt minus 1pt
\def\plaintexoption#1{\par\setbox0\vbox to 0pt{\vskip-2ex\vss
    \hfill\small\textcolor{gray}{\hologo{plainTeX}\unskip:}\,#1}\par
  \AddToHookNext{para/begin}{\vadjust{\box0}}}
\item[InterHangul] %\pkgkwd*{InterHangul}%
    {\addhangulfontfeature{InterHangul=.5ex}한글 글자 사이의 자간}.
    아래는 $-0.04$em 만큼 한글 자간을 지시한다.
    \plaintexoption{|interhangul=|\meta{dimen}}
\begin{verbatim}
    [InterHangul=-0.04em]
\end{verbatim}

\item[InterLatinCJK] %\pkgkwd*{InterLatinCJK}%
  한글^^b7한자와 Latin문자 사이의 자간을 설정한다.
    한글^^b7한자 다음에 라틴 문자가 오는 경우 원래 줄바꿈이 되지 않지만
    이 옵션을 주면 줄바꿈이 가능해진다.
  \plaintexoption{|interlatincjk=|\meta{dimen}}
\begin{verbatim}
...
\item[InterCharacter] %\pkgkwd*{InterCharacter}%
  ...
  \plaintexoption{|intercharacter=|\meta{dimen}}
\begin{verbatim}
    [InterCharacter=.125em]
\end{verbatim}

\item[InterCharStretch] %\pkgkwd*{InterCharStretch}%
  CJK 글자간 가변공백\hparen{글루}의 stretch\hparen{늘이기} 값을 지시한다.
  \plaintexoption{|intercharstretch=|\meta{dimen}}
\begin{verbatim}
    [InterCharStretch=0.5pt]
\end{verbatim}

\item[InterCharPenalty] %\pkgkwd*{InterCharPenalty}%
  CJK 글자간 가변공백\hparen{글루} 직전에 삽입되는 penalty의 값을 지시한다.
  |-10000|부터 |10000|까지의 값을 줄 수 있는데,
  양수값은 글자간 개행을 억제하고 음수값은 글자간 개행을 권장한다.
  기본값은 |50|으로, 글자간 개행을 조금 억제하여 단어간 개행을 조금 선호하는 결과를 낳는다.
  옵션값을 |0|으로 주면 이러한 차이가 사라져 글자간 개행이 더 빈번하게 일어난다.
  \plaintexoption{|intercharpenalty=|\meta{number}}
\begin{verbatim}
    [InterCharPenalty=0]
\end{verbatim}
...
\item[CharRaise] %\pkgkwd*{CharRaise}%
  ...
  \plaintexoption{|charraise=|\meta{dimen}}
\begin{verbatim}
    [CharRaise=2pt]
\end{verbatim}
  세로쓰기에서는 모든 글자들을 지정한 값만큼 오른쪽으로 이동시킨다.%
  \footnote{세로쓰기에서는 이 옵션값을 고정길이\hparen{가령 |pt| 단위}로 주어야
    크기가 다른 폰트들 간에 중앙정렬이 유지된다. }

\def\plaintexoption#1#2{{\unskip\nobreak\hfil\penalty50\hskip2em
  \null\nobreak\hfil\raise#2\hbox{\small\textcolor{gray}{\hologo{plainTeX}\unskip:}\,#1}%
  \parfillskip=0pt \finalhyphendemerits=0 \par}}
\item[RemoveClassicSpaces] %\pkgkwd*{RemoveClassicSpaces}%
  고문헌 조판시에 CJK 글자들 사이의 공백을 없애준다.
  \plaintexoption{|+removeclassicspaces|}{2pt}

\item[CompressPunctuations] %\pkgkwd*{CompressPunctuations}%
  CJK 구두점\hparen{낫표 따위}의 글자폭을 반각으로 만든다.
  버전 2.0부터는 사용자가 지시하지 않는 한 글자폭을 자동으로 줄여주지 않는다.
  이 옵션은 |CharacterWidth=|\allowbreak|AlternateHalf| 내지 \hparen{세로쓰기의 경우}
  |Vertical=|\allowbreak|HalfMetrics|와 사실상 같은 기능을 한다.%
  \footnote{플레인텍에서는 각각 |+halt| 및 |+vhal|이다.  이들과 완전 동일하진
    않다. 가령 U+00B7\hparen{^^b7} 및 고문헌조판 모드에서
    U+2018\hparen{^^^^2018} U+2019\hparen{^^^^2019} U+201C\hparen{^^^^201c} U+201D\hparen{^^^^201d}
    처리의 경우 미세한 간격 차이가 있을 수 있다. }
  그래서 이들 옵션 중 하나가 지시되어 있다면 그것이 우선 적용되고
  |CompressPunctuations|는 \hparen{설령 명시되었더라도} 아무 작용도 하지 않는다.%
  \footnote{그러나 HarfBuzz 모드 세로쓰기에서는 |vhal| 대신 |CompressPunctuations|가 사용된다.}
  \plaintexoption{|+compresspunctuations|}{2pt}

\item[Protrusion] %\pkgkwd*{Protrusion}%
  특정 글자가 행 끝에 왔을 때 판면 바깥으로 끌어내는
  기능이다. \luatex은 기본으로 온점 반점 등을 완전 글자내밀기 한다.
  |Protrusion|은 |Protrusion=|\allowbreak|default|와 같은 뜻이다.
  자신만의 설정을 만들어 지정할 수 있다.%
  \footnote{default 설정을 고치는 방법은 이를테면 다음과 같다%
    \hparen{또한 이 매뉴얼의 소스 앞부분을 참고하라}:\par
    \leftskip=3em\noindent
    \cs{directlua{ fonts.protrusions.setups.default[0x201C] = { 1, 0 } }} }
  \plaintexoption{|protrusion=default|}{6pt}

\item[Expansion] %\pkgkwd*{Expansion}%
  판면의 균일한 조판을 위해 글자들을 미세하게 늘이거나
  줄이는 기능이다. |Expansion|은 |Expansion=|\allowbreak|default|와 같은 뜻이다.
  \plaintexoption{|expansion=default|}{0pt}
...
그림~\ref{fig:gomun} 참조.
버전 2.0부터는 고문헌 조판에서 한글^^b7한자와 영문자가 붙어 나오더라도 자동으로
간격을 조정해주지 않으므로 사용자가 |InterLatinCJK| 옵션을
지정해야 한다.
또한 불필요한 공백이 오더라도 자동으로 제거해주지 않으므로
...
영\hparen{零}으로 강제하기 위해선 \pkgkwd{\inhibitglue} 명령을 이용한다.

\def\analectstext{\addhangulfontfeature{RemoveClassicSpaces}
子 曰：「學而時習之，不亦說乎？有朋自遠方來，不亦樂乎？人不知而不慍，不亦君子乎？」\par
有子 曰：「其爲人也孝弟，而好犯上者，鮮矣！不好犯上，而好作亂者，未之有也！君子務本，本立而道生；孝弟也者，其爲仁之本與？」\par
子 曰：「巧言令色，鮮矣仁！」\par
曾子 曰：「吾日三省吾身：爲人謀，而不忠乎？與朋友交，而不信乎？傳，不習乎？」\par
子 曰：「道千乘之國，敬事而信，節用而愛人，使民以時。」\par
子 曰：「弟子入則孝，出則弟；謹而信，汎愛衆；而親仁，行有餘力，則以學文。」
}
\begin{figure}
  \centering
  ...
  \caption{고문헌 조판 보기. typesetclassic 환경을 이용했다.
    글꼴에 |CompressPunctuations|와 |RemoveClassicSpaces| 옵션을 주었다.
  }\label{fig:gomun}
\end{figure}

\begin{plaintex}
  고문헌 조판을 하려면 \hparen{문서 전체에 적용하지 않는다면 그룹 안에서}
  \cs{typesetclassic}.

  잠깐 현대 한국어를 조판하려면 \hparen{그룹 안에서}
  \cs{typesetmodern}.
\end{plaintex}
...
\footnote{%
  ...
  |otfinfo -t| \meta{파일} 명령으로 글꼴에 들어있는 테이블 정보를 알 수 있다. }\,%
\footnote{%
  폰트 옵션 |Renderer=HarfBuzz| 또는 |Renderer=OpenType| 등%
  \hparen{플레인텍에서는 |mode=harf| 옵션}을 주고
  컴파일하여 HarfBuzz 모듈을 이용하는 경우에도 세로쓰기가 \hemph{지원된다}.
  그러나 |Renderer=Node|를 이용한 세로쓰기에 비해 속도가 느리고 아직은 실험 단계에 머물러 있다.
  다만, 혹시 |extend| \hparen{\LaTeX에서는 |FakeStretch|}나 |squeeze| 옵션으로
  글자를 늘이거나 짜부라지게 하려면 이는 현재 HarfBuzz 모드에서만 정상 작동됨에 유의하라.}\,%
\footnote{%
...
|\begin{|\pkgkwd{vertical}|}{|\meta{dimen}|}| \ldots\ |\end{vertical}|
환경을 이용한다. 인자 \meta{dimen}으로 세로쓰기 박스의 높이를 지시한다.
...
\begin{figure}
  ...
  \caption{세로쓰기의 예. 박스 높이 |20em|을 지시했다.
    권장하지 않는다지만 |Renderer=HarfBuzz|를 세로쓰기 폰트 옵션에 넣어봤다.
    }\label{fig:vertical}
\end{figure}
...
\begin{figure}
  ...
  \caption{세로쓰기의 다른 예. 박스 높이 |17em|을 지시했다.
    글꼴에 |CompressPunctuations| 및 |RemoveClassicSpaces|와 더불어
...
\begin{figure}
  ...
  \framebox[\linewidth]{
    \begin{vertical}{17em}
      \linespread{1.5}\sffamily \verticalhangulfont
      \addhangulfontfeature{CharacterWidth=Full}
      \parindent0pt \everypar{\hangindent1em \hangafter1 }
  ...
  \caption{세로쓰기의 또 다른 예.  박스 높이 |17em|을 지시했다.
    HarfBuzz 모드에서 Noto Sans CJK 글꼴의 세로쓰기 공백은 원래 |1em|,
    즉 전각크기로 매우 크기 때문에, 여기서는 \cs{spaceskip} 명령을 이용하여
    반각 크기로 강제했다. 아스키 괄호를 전각으로 바꾸기 위해
    |CharacterWidth=Full| 옵션도 추가로 지시했는데,
    이 옵션은 Node 모드에서도 공백 크기에 영향을 미칠 수 있다.
  }\label{fig:vertical2}
\end{figure}
...
|\begin{|\pkgkwd{horizontal}|}{|\meta{dimen}|}| \ldots\ |\end{horizontal}|
환경을 이용한다. \meta{dimen}은 가로쓰기 박스의 너비를 지시한다.
만약 인자가 비어있거나 \cs{empty}이면 본문을 한 줄짜리 박스로 식자하며,
이때 너비는 당연히 natural width가 된다.
\end{itemize}

\begin{plaintex}
  문서 일부 세로쓰기는 |\vertical{|\meta{dimen}|}| \ldots\ |\endvertical|.
  인자는 비워둘 수 있다.

  문서 전체 세로쓰기는 문서 첫머리에 \cs{verticaltypesetting}.

  몇몇 페이지의 세로쓰기는 \cs{beginverticaltypesetting} \ldots\ \cs{endverticaltypesetting}.

  세로쓰기 중 가로쓰기는 |\horizontal{|\meta{dimen}|}| \ldots\ |\endhorizontal|.
  인자는 비워둘 수 있다.
\end{plaintex}
...
\pkgkwd{\xxruby}|{...}{...}| 명령을 사용한다.\footnote{%
  |lua-uni-algos| 패키지를 설치해두어야 한다.
}
글자들 사이에 줄바꿈이 \xxruby{허용}{許容}된다.
두 인자의 글자 수가 동일해야 한다.

\section{밑줄긋기}\label{sec:uline}

ulem 패키지가 \luatexko와 궁합이 맞지 않아\hparen{줄바꿈에 문제가 있음}
명령을 따로 제공한다.
%\pkgkwd*{\uline}%
%\pkgkwd*{\sout}%
%\pkgkwd*{\uuline}%
...
\begin{sourceandresult}
  \setbox0\hbox{\ttfamily\string\dotuline\{밑줄을 점선으로 긋는다\}}\dimen0=\wd0
  \def\srcres#1#2{\leavevmode \hbox to\dimen0
    {\ttfamily\string#1\{#2\}\hss}\quad ⇒\quad #1{#2}\par}
  \srcres \uline{밑줄을 그을 수 있다}
  \srcres \uuline{밑줄을 두 줄 긋는다}
  \srcres \uwave{물결표로 밑줄을 삼는다}
  \srcres \sout{취소선을 그을 수 있다}
  \srcres \xout{빗금으로 취소할 수 있다}
  \srcres \dashuline{대시로 밑줄을 삼는다}
  \srcres \dotuline{밑줄을 점선으로 긋는다}
\end{sourceandresult}
...
\tag_if_active:T
{
  \tagpdfsetup
  {
    role/new-attribute = {TextDeco-HalfYellowBg}{/O~/Layout~/BackgroundColor~[1.0~1.0~0.5]},
  }
  \NewTaggingSocketPlug{luatexko/markoverwith/begin}{yellowmarker}
  {
    \leavevmode
    \tag_mc_end_push:
    \tag_struct_begin:n{tag=Em, attribute=TextDeco-HalfYellowBg}
    \tag_mc_begin:n{}
  }
}
\cs_new_nopar:Npn \ymarker
{
  \tag_if_active:T
  {
    \begingroup
    \AssignTaggingSocketPlug{luatexko/markoverwith/begin}{yellowmarker}
    \AddToHookNext{cmd/luatexkomarkoverwith/after}{\endgroup}
  }
  \markoverwith{\hbox{{\color{yellow!50}\vrule~width~1pt~height~9pt~depth~2pt}}}
}
\ExplSyntaxOff
\begin{sourceandresult}
  \srcres{\ymarker{노란색을 칠해서 강조한다}}
\end{sourceandresult}
...
\begin{sourceandresult}
  \def\setjprn#1{\leavevmode\texttt{\string#1}#1\par}
  \setjprn{\josaignoreparens=1}
  \srcres{홍길동(1992)\는}
  \srcres{홍길동(2001)\로}
  \setjprn{\josaignoreparens=0}
  \srcres{홍길동(1992)\는}
  \srcres{홍길동(2001)\로}
\end{sourceandresult}
...
\footnote{수직 모드\hparen{문단과 문단 사이}에서 사용할 것.
...
\begin{sourceandresult}
  \srcres{$가^{나^다}$}
\end{sourceandresult}
수식 한글 폰트를 설정하면 수식 모드에서도 한글을 \hparen{hbox로 감싸지 않고}
직접 입력할 수 있다.%
\footnote{%
  \cs{hbox}를 쓰지 않고 직접 입력하는 수식 한글이
...
버전 2.0부터는 자동으로 수식 한글을 잡아주지 않는다.%
...
\kotex과 동일하게 항목 번호를 한국어 기호로 붙일 수 있다.
  ...
  \cs{hroman} \cs{hRoman} \cs{hNum} \cs{hanjanum} 따위를 지원한다%
  \hparen{|cjk-ko| 문서 참조}.

  예컨대 \cs{onum{page}}라고 하면 쪽번호가 \onum{page}\로 찍힌다.
  따라서 가령 제2단계 enumerate 환경의 항목번호 양식을 바꾸려면
  \cs{def}\allowbreak\cs{labelenumii}\allowbreak|{\onum{enumii}}|처럼 선언할 수 있다.
  또한 \cs{pagenumbering}\allowbreak|{onum}|처럼 선언해서 쪽번호 양식을 바꿀 수도 있다.
...
\XeTeX의 \cs{XeTeXinputnormalization} 명령과 유사하다.

\section{UHC 입력 인코딩}\label{sec:uhcencoding}

권장하지 않지만 불가피하게 입력 인코딩이 UHC \hparen{Unified Hangul Code}%
\footnote{%
...
\begin{itemize}\linespread{1.1}\small
  \item[v5.5] ------
  \item \hyperref[sec:fontoption]{글꼴 옵션 |InterCharPenalty| 추가}
  \item[v5.0] ------
  \item \hyperref[sec:ruby]{\cs{xxruby} 명령 개선} \hparen{|lua-uni-algos| 패키지 설치 필요}
  \item[v4.9] ------
  \item \hyperref[sec:verttype]{HarfBuzz 폰트의 세로쓰기 지원} \hparen{권장하지는 않음}
  \item[v4.0] ------
  \item \LaTeX 의 Tagged PDF 기능 지원
  \item[v3.2] ------
  \item 한자 뒤에서도 자동조사 기능이 대체로 동작한다.
    \hparen{|xetexko| 패키지 설치 필요}
  ...
  \item 기본 로드되는 고정폭 한글 폰트\hparen{UnDotum}의 폭을
  ...
  \item[v2.5] ------
  \item LuaHB\TeX\ 지원 \hparen{세로쓰기 제외}
