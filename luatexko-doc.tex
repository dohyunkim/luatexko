...
\directlua{
  if tex.formatname:find"luahblatex" then
    token.set_macro("luatexkoHBrenderer","Renderer=OpenType")
  else
    token.set_macro("luatexkoHBrenderer")
  end
}
\newhangulfontfamily\testharfbuzz{Noto Serif CJK KR}[
  \luatexkoHBrenderer,
  Script=Hangul,
  Language=Korean,
  UprightFont=* Regular,
...
\footnote{%
  정확히 말하자면 \cs{language=1}, 즉 nohyphenation 상황에서는
  작동하지 않는다.
...
\begin{figure}
  \fboxsep=1em
  \fbox{\parbox{\dimexpr\linewidth-2.1em\relax}{\testharfbuzz
  \parindent1em
  \hunmintxt
}}
\caption{옛한글 조판 보기. Harfbuzz 이용.}\label{fig:yethangul}
\end{figure}
