...
\directlua{
  ...
  function(data)
    if type(data) == "table" and data.hb then else return end
    local expansion  = data.specification.features.normal.expansion
    local protrusion = data.specification.features.normal.protrusion
    if expansion == "default" then
      data.stretch = 20
      data.shrink  = 20
      data.step    = 1
    end
    if protrusion == "notoserifcjk" then
      local quad = data.parameters.quad
      for i,v in pairs(notoserifcjk_kr) do
        local char = data.characters[i]
        char.left_protruding  = char.width/quad * v[1] * 1000
        char.right_protruding = char.width/quad * v[2] * 1000
      end
    end
  end, "luatexko.harf_expansion_protrusion")
}
...
\item[Protrusion] \pkgkwd*{Protrusion}%
  ...
  Lua\TeX은 기본값으로 온점 반점 등을 완전 글자내밀기 한다.
