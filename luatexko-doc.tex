...
\cs_new_nopar:Npn \fakespaceoff #1
{
  \tag_if_active:TF
  {
    \tag_spacechar_off:
...
\def\xetexko{\XeTeX\artifacttag{-}\logoko}
...
\def\hparenstop{\ifx\@let@token\@sptoken\else
  \ifx\@let@token\footnote\nobreak\else\ifx\@let@token ,\nobreak\fi\fi
  \fakespaceoff{\hskip.1em plus.01em\relax}\fi\endgroup}
...
\footnote{%
  정확히 말하자면 |\\language=\\l@nohyphenation| \hparen{플레인텍에서는 |\\language=-1|인 경우},
...
\item[CompressPunctuations] \pkgkwd*{CompressPunctuations}%
  ...
  \footnote{그러나 HarfBuzz 모드 세로쓰기에서는 |CompressPunctuations|가 우선권을 갖는다.}
...
\begin{plaintex}
  문서 일부 세로쓰기는 |\vertical{<dimen>}| \ldots\ |\endvertical|.
  인자는 비워둘 수 있다.
...
\begin{itemize}\linespread{1.1}\small
  \item[v5.0] ------
  \item \hyperref[sec:ruby]{\cs{xxruby} 명령 개선} \hparen{|lua-uni-algos| 패키지 이용}
  \item[v4.9] ------
