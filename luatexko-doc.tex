...
\begin{description}
  \item[InterHangul] \pkgkwd*{InterHangul}%
    ...
    \footnote{%
      플레인텍에서는 |interhangul=|\meta{dimen}. }
\begin{verbatim}
    [InterHangul=-0.04em]
\end{verbatim}
...
\item[InterLatinCJK] \pkgkwd*{InterLatinCJK}%
  한글^^b7한자와 Latin문자 사이의 자간을 설정한다.%
    \footnote{%
      플레인텍에서는 |interlatincjk=|\meta{dimen}. }
...
\item[InterCharacter] \pkgkwd*{InterCharacter}%
  ...
  \footnote{%
    플레인텍에서는 |intercharacter=|\meta{dimen}. }
...
\item[InterCharStretch] \pkgkwd*{InterCharStretch}%
  CJK 글자간 가변공백\hparen{글루}의 stretch 값을 지시한다.%
  \footnote{%
    플레인텍에서는 |intercharstretch=|\meta{dimen}. }
\begin{verbatim}
...
\item[InterCharPenalty] \pkgkwd*{InterCharPenalty}%
  ...
  \footnote{플레인텍에서는 |intercharpenalty=|\meta{number}. }
...
\item[CharRaise] \pkgkwd*{CharRaise}%
  ...
  \footnote{%
    플레인텍에서는 |charraise=|\meta{dimen}. }
\begin{verbatim}
    [CharRaise=2pt]
\end{verbatim}
  세로쓰기에서는 모든 글자들을 지정한 값만큼 오른쪽으로 이동시킨다.%
  \footnote{%
    세로쓰기에서는 이 옵션 값을 고정길이\hparen{가령 |pt| 단위}로 주어야
...
\footnote{%
  플레인텍에서는 \hparen{문서 전체에 적용하지 않는다면 그룹 안에서}
