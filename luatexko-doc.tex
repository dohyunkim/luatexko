...
\begin{quote}\small
  ...
  \begin{itemize}\itemsep=0pt
    ...
    \item 플레인텍에서도 luatexko.sty을 불러야 한다.
      \hyperref[sec:loading]{☞}
    \item 패키지 옵션 |unfonts|가 선언되지 않으면 은글꼴을 자동으로
      부르지 않는다.
      \hyperref[sec:packageopt]{☞}
    \item 글꼴 옵션값을 |<dimen>|으로 지시한다.
      \hyperref[sec:fontoption]{☞}
    \item \MP\ 코드를 넣으려면 luamplib 패키지를 불러야 한다.
      \hyperref[sec:metapost]{☞}
    \item[v1.3] ------
    \item fallbackfont 계열의 명령이 추가되었다.
      \hyperref[sec:fontcmds]{☞}
    \item |\hangulpunctuations| 선언이 추가되고,
      |QuoteRaise| 옵션이 없어졌다.
      \hyperref[sec:fontswitch]{☞}
    \item |\josaignoreparens=1|이 선언되면 자동조사 결정시 괄호 부분을
      무시한다.
      \hyperref[sec:autojosa]{☞}
    \item[v1.5] ------
    \item 세로쓰기를 지원하기 위한 실험적 코드가 들어갔다.
      \hyperref[sec:verttype]{☞}
    \item |\actualtext| 명령이 추가되었다.
      \hyperref[sec:actualtext]{☞}
    \item 한글·한자 글꼴의 지정이 없으면 나눔 Type1을 이용한다.
      \hyperref[sec:fontswitch]{☞}
  \end{itemize}
\end{quote}
...
폰트에서} 글자를 찾아 찍는다. 세 가지 모두 지정되지 않았다면 나눔 Type1
...
|[hangul]| 옵션을 주면 |\hangulpunctuations=1|이 자동으로
...
\sffamily \def\hellipsis{$\cdots\cdots$}
\hangulfontspec[RawFeature=vertical;+vert;+vhal]{HCR Dotum LVT}
...
경우 |vert| 자질을 명시해주어야 한다. 둘 다 없으면 세로쓰기에 적합지 않은
글꼴이다.
세로쓰기에는 \luatex\ 0.79 이상, 즉 \texlive\ 2014 이후 버전이 필요하다.

문서의 일부를 세로쓰기하려면 |\begin{vertical}{<dimen>}...\end{vertical}|
환경을 이용하라. |<dimen>|으로 세로쓰기 박스의 높이를 지시한다.
그림~\ref{fig:vertical} 참조.
플레인텍에서는 |\vertical{<dimen>} ... \endvertical|.
...
전처리부에서 |\usepackage{luamplib}|을 선언하면 \MP\ 코드를 문서 중간에
...
①~|\rubyfont|: 루비를 식자할 폰트를 지시해 둔다. 기본값은 현재 폰트.
...
|\actualtext{...}| 명령은 인자를 식자함과 동시에, 입력한 문자 그대로
PDF에서 텍스트로 추출할 수 있게 해준다. 인자가 두 페이지에 나눠지지 않도록
유의한다.  모든 PDF 리더가 이를 지원하는 것은 아니다. 예:
$\actualtext{\sqrt 2}$,
{\fontspec[Script=Hangul,Ligatures=Required]{HCR Dotum LVT}
\actualtext{ᄆᆞᄎᆞᆷ〮내〯}}.
인자가 글자 없이 그림으로만 돼있다면 |\actualtext*{...}| 방식을 이용한다.
